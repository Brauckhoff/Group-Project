...
def main(argv):
    ...
    for alignFilename in alignFilenames:
        ...
        # modification to deal with division through zero Error
        size = os.path.getsize(alignFilename)
        if size == 0: continue
        ...
        for line in open(alignFilename):
            ...
            #print(f'content of alignFile: {fields}')
            ...
            print(f'Reference Name: {referenceName}')
            print(f'alignment lenght: {alignLength}')
            if referenceName in validReferenceNames: print("Passed checkpoint 1")
...
def collectCircularContigs(assembler, inputFilename, minContigLength):
    ...
    if assembler == "mdbg":
        circularContigs = collectCircularContigs_mdbg(inputFilename, minContigLength, True)
...
def collectCircularContigs_mdbg(inputFilename, minContigLength, circularContigOnly):
...
def computeANI(tmpDir, referenceFilename, mappedContigs, contigFilename):
    ...
    print("--- Debug ANI ---")
    print(f"Reference File: {referenceFilename}")
    print(f"Contigs: {contigFilename}")

    for header, seq in SimpleFastaParser(fileHandle):
        print(f"header: {header.split(' ')[0]}")
        print(f"mappedContigs: {mappedContigs}")
        
        if not header.split(' ')[0] in mappedContigs: continue
        contigFile.write(">" + header + "\n")
        contigFile.write(seq + "\n")

    contigFile.close()
    
    command = "conda run average_nucleotide_identity.py -i " + aniInputDir + " -o " + aniResultDir
    os.system(command)
